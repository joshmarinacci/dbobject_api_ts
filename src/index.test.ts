...
async function create_node_test() {
  ...
  // insert new node
  let node1_uuid = await store.new_object({
    name: "first node",
    docuuid: doc1_uuid,
    props: {},
  });
...
async function node_versioning_test() {
...

  {
    ...
    let old_node1_result: JDResult = await store.get_object_by_version(
      bart_uuid,
      0,
    );
    assert_eq(
      "node previous value",
      old_node1_result.data[0].props.simpson,
      "bart",
    );
    // confirm new node still has the right value
    let old_node2_result: JDResult = await store.get_object_by_version(
      bart_uuid,
      1,
    );
    assert_eq(
      "node previous value",
      old_node2_result.data[0].props.simpson,
      "lisa",
    );
  }
  //
...
async function image_attachments_test() {
  ...
  let add_res = await store.add_attachment(
    obj_res.data[0].uuid,
    "pdf",
    att_res.data[0],
  );
...
async function complex_query_test() {
  ...
  await store.new_object({
    type: "bookmark",
    contents: "some cool java is here",
  });
  await store.new_object({
    type: "bookmark",
    contents: "some cool Javascript is here",
  });
  ...
  assert_eq("query returned one object", res.data.length, 1);
  assert_eq(
    "obj has correct value",
    res.data[0].props.contents,
    "some cool Javascript is here",
  );
  //destroy
  await store.destroy();
}
